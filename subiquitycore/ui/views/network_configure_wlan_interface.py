...
class NetworkList(WidgetWrap):
    def __init__(self, parent, ssids):
        ...
        ssid_list = [
            menu_btn(label=ssid, on_press=self.do_network) for ssid in ssids if ssid
        ]
...
class NetworkConfigureWLANStretchy(Stretchy):
    ...
    def show_ssid_list(self, sender):
        self.parent.show_overlay(
            NetworkList(self, self.dev_info.wlan.visible_ssids), width=60
        )
    ...
    def _build_iface_inputs(self):
        ...
        if len(visible_ssids) > 0:
            networks_btn = menu_btn(
                "Choose a visible network", on_press=self.show_ssid_list
            )
        else:
        ...
        warning = "Only open or WPA2/PSK networks are supported at this time."
        col = [
            Text(warning),
            Text(""),
            self.ssid_row,
    ...
    def update_link(self, dev_info):
        if dev_info.name != self.dev_info.name:
            return
        self.dev_info = dev_info
        self.inputs.contents = [
            (obj, ("pack", None)) for obj in self._build_iface_inputs()
        ]
