...
class FilesystemController(SubiquityController, FilesystemManipulator):
    ...
    def _is_windows_loopback_boot(self):
        """Check if we booted via the Windows installer's GRUB loopback.

        When the Windows installer sets up the boot, it passes
        'akash.from-windows' on the kernel command line.  In this flow the
        ISO is loop-mounted from the NTFS partition and the normal systemd
        shutdown sequence deadlocks trying to unmount the loop device,
        producing endless 'i/o error, dev loop0' messages.
        """
        try:
    ...
    async def _pre_shutdown(self):
        ...
        """
        # When booted from the Windows installer via GRUB loopback, the
        # ISO is loop-mounted from NTFS with casper/squashfs layers on
        # top.  ANY filesystem operation (unmount, sync, remount-ro) can
        # deadlock the kernel because of the loop device dependency
        # chain.  The only safe action is an immediate hard reboot.
        # By this point all install data has been written by curtin and
        # logs have been copied to /target, so no data is at risk.
        if self._is_windows_loopback_boot():
            log.debug("Windows loopback boot detected — forcing "
                      "immediate hard reboot via sysrq")
            try:
