...
class HomenodeTokenForm(Form):
    ...
    token = TokenField(
        _("Installation Key:"),
        help=_(
            "Enter five words separated by dashes (e.g., word1-word2-word3-word4-word5)"
        ),
    )
...
class HomenodeTokenView(BaseView):
    ...
    def __init__(self, controller, token: str, has_network: bool):
        ...
        if has_network:
            excerpt = _(
                "Enter your Homenode installation key consisting of five words separated by dashes."
            )
        else:
            excerpt = _(
    ...
    def _validate_and_submit(self, token: str):
        ...
        log.info(
            "_validate_and_submit called for token: %s",
        ...
        def on_failure(status, message):
            log.warning(
                "Token validation failed: status=%s, message=%s", status, message
            )
            self.remove_overlay()
        ...
        log.info(
            "About to call controller.check_token with token length: %d", len(token)
        )
        try:
    ...
    def _show_validation_error(self, status: HomenodeTokenCheckStatus, message: str):
        ...
        if status == HomenodeTokenCheckStatus.INVALID_TOKEN:
            ...
            error_msg = message or _(
                "Failed to verify installation key. Please try again."
            )
