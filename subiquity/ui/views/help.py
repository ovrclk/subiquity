...
def close_btn(app, stretchy):
    return other_btn(
        _("Close"), on_press=lambda sender: app.remove_global_overlay(stretchy)
    )
    ...
    if ssh_info is not None:
        if len(ssh_info.ips) > 1:
            ...
            for ip in ssh_info.ips:
                texts.append(
                    Text("{}@{}".format(ssh_info.username, ip), align="center")
                )
        else:
            texts.append(
                _(SSH_HELP_ONE_ADDRESSES).format(
        ...
        if ssh_info.authorized_key_fingerprints:
            if len(ssh_info.authorized_key_fingerprints) == 1:
                ...
                texts.append(
                    Text(
                        _(SSH_HELP_EPILOGUE_ONE_KEY).format(
                ...
                for key in ssh_info.authorized_key_fingerprints:
                    rows.append(TableRow([Text(key.keytype), Text(key.fingerprint)]))
                texts.append(TablePile(rows))
            if ssh_info.password_kind == PasswordKind.KNOWN:
                texts.append("")
                texts.append(
                    SSH_HELP_EPILOGUE_KNOWN_PASS_KEYS.format(password=ssh_info.password)
                )
            ...
            if ssh_info.password_kind == PasswordKind.KNOWN:
                texts.append(
                    SSH_HELP_EPILOGUE_KNOWN_PASS_NO_KEYS.format(
    ...
    def __init__(self, app):
        ...
        if app.opts.dry_run:
            ...
            for key, text in DRY_RUN_KEYS:
                rows.append(TableRow([Text(_(key)), Text(_(text) + " " + dro)]))
...
class OpenHelpMenu(WidgetWrap):
    def __init__(self, parent):
        ...
        if local_title is not None:
            local = menu_item(
                local_title, on_press=self.parent.show_local(local_title, local_doc)
            )
            buttons.add(local)
        else:
        ...
        self.width = (
            max([widget_width(b) for b in entries if not isinstance(b, Divider)]) + 2
        )
